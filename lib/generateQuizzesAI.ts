// TypeScript mirror of the offline generator in scripts/generate-quizzes.js.
// This is intended for programmatic use or future tooling integrations.

import type { LocalizedText, Quiz, Question } from "./types";
...
function localizeAll(str: string): LocalizedText {
  return { en: str, am: str, fr: str };
}

function makeTitle(category: string, index: number): LocalizedText {
  const templates = [
    `Only 1 in 50 people can pass this ${category} test 🤯`,
    `Can you score 8/10 on this ${category} challenge? 💥`,
    `Do you really remember your ${category}? Take the quiz! ✅`,
...
function makeDescription(category: string): LocalizedText {
  return localizeAll(
...
function makeQuestions(category: string): Question[] {
  return [
    {
...
export function generateQuizzes(totalPerCategory = 20): Quiz[] {
  const quizzes: Quiz[] = [];
